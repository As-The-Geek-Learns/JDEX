 ...
 * ======================
 * Admin interface for database browsing and SQL execution.
 */
...
/**
 * Props for the DatabaseTab component
 */
...
function DatabaseTab({ onDataChange }: DatabaseTabProps): JSX.Element {
  const [sqlQuery, setSqlQuery] = useState('');
  const [sqlResult, setSqlResult] = useState<SQLResult | null>(null);
  const [selectedTable, setSelectedTable] = useState('');
  const [tableData, setTableData] = useState<TableData>({ columns: [], rows: [] });

  const handleExecuteSQL = (): void => {
    ...
    const result = executeSQL(sqlQuery) as SQLResult;
    setSqlResult(result);
  ...
  const handleTableSelect = (tableName: string): void => {
    ...
    const data = getTableData(tableName) as TableData;
    setTableData(data);
  };
