 ...
 */
type FeedbackTypeId = 'bug' | 'feature' | 'design' | 'general';

/**
 * Color scheme identifier
 */
type FeedbackColor = 'red' | 'amber' | 'purple' | 'teal';
...
/**
 * Props for FeedbackTypeCard component
 */
interface FeedbackTypeCardProps {
  type: FeedbackType;
  isSelected: boolean;
  onSelect: (id: FeedbackTypeId) => void;
}
...
function getColorClasses(color: FeedbackColor): ColorClasses {
  const colors: Record<FeedbackColor, ColorClasses> = {
    red: {
      bg: 'bg-red-500/10',
...
function buildEmailUrl(type: FeedbackTypeId, message: string): string {
  const feedbackType = FEEDBACK_TYPES.find((t) => t.id === type);
  const subject = encodeURIComponent(
    `${feedbackType?.subjectPrefix || '[Feedback]'} JDex v${APP_VERSION}`
  );
...
function FeedbackTypeCard({ type, isSelected, onSelect }: FeedbackTypeCardProps): JSX.Element {
  ...
  return (
    ...
    >
      ...
      {isSelected && (
        <div
          className={`absolute top-2 right-2 w-2 h-2 rounded-full bg-gradient-to-br ${colors.gradient}`}
        />
      )}
    </button>
  );
    ...
    <div className="p-6 max-w-2xl">
      ...
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-white flex items-center gap-3">
          <span
            className="w-10 h-10 rounded-xl bg-gradient-to-br from-teal-500 to-purple-500
      ...
      <div className="glass-card p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-slate-400 mb-2">Send your feedback to:</p>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-purple-200/70">
                For visual feedback, attach screenshots to your email showing what you like or what
                you'd change. You can also share links to designs that inspire you!
              </p>
            </div>
          </div>
        </div>
