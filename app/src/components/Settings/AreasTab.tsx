 ...
 * Areas Tab Component
 * ===================
 * CRUD interface for managing Johnny Decimal areas.
 */
...
import type { Area } from '../../types/index.js';
...
/**
 * New area form data (string values for inputs)
 */
interface NewAreaFormData {
  range_start: string;
  range_end: string;
  name: string;
  description: string;
...
/**
 * Editing area data (string values for inputs)
 */
...
/**
 * Props for the AreasTab component
 */
...
function AreasTab({ areas, onDataChange }: AreasTabProps): JSX.Element {
  const [editingArea, setEditingArea] = useState<EditingArea | null>(null);
  const [newArea, setNewArea] = useState<NewAreaFormData>({
    range_start: '',
    range_end: '',
    name: '',
    description: '',
  ...
  const handleUpdateArea = (area: EditingArea): void => {
    try {
  ...
  const handleDeleteArea = (id: number): void => {
  ...
  const handleNewAreaChange = (field: keyof NewAreaFormData, value: string): void => {
    setNewArea({ ...newArea, [field]: value });
  };

  const handleEditingAreaChange = (field: keyof EditingArea, value: string): void => {
    if (!editingArea) return;
    setEditingArea({ ...editingArea, [field]: value });
  };

  const startEditing = (area: Area): void => {
    setEditingArea({
      id: area.id,
      range_start: String(area.range_start),
      range_end: String(area.range_end),
      name: area.name,
      description: area.description || '',
      color: area.color,
    });
        ...
        <div className="grid grid-cols-6 gap-3">
          <input
            ...
            onChange={(e: ChangeEvent<HTMLInputElement>) =>
              handleNewAreaChange('range_start', e.target.value)
            }
          ...
          <input
            ...
            onChange={(e: ChangeEvent<HTMLInputElement>) =>
              handleNewAreaChange('range_end', e.target.value)
            }
            className="bg-slate-800 border border-slate-600 rounded px-3 py-2 text-white text-sm"
          />
          <input
            ...
            value={newArea.name}
            onChange={(e: ChangeEvent<HTMLInputElement>) =>
              handleNewAreaChange('name', e.target.value)
            }
            className="bg-slate-800 border border-slate-600 rounded px-3 py-2 text-white text-sm"
          />
          <input
            ...
            onChange={(e: ChangeEvent<HTMLInputElement>) =>
              handleNewAreaChange('description', e.target.value)
            }
            className="bg-slate-800 border border-slate-600 rounded px-3 py-2 text-white text-sm"
          />
          <input
            ...
            value={newArea.color}
            onChange={(e: ChangeEvent<HTMLInputElement>) =>
              handleNewAreaChange('color', e.target.value)
            }
            ...
            {editingArea?.id === area.id ? (
              <>
                <input
                  ...
                  onChange={(e: ChangeEvent<HTMLInputElement>) =>
                    handleEditingAreaChange('range_start', e.target.value)
                  }
                ...
                <input
                  ...
                  onChange={(e: ChangeEvent<HTMLInputElement>) =>
                    handleEditingAreaChange('range_end', e.target.value)
                  }
                  className="w-16 bg-slate-800 border border-slate-600 rounded px-2 py-1 text-white text-sm"
                />
                <input
                  ...
                  onChange={(e: ChangeEvent<HTMLInputElement>) =>
                    handleEditingAreaChange('name', e.target.value)
                  }
                ...
                <input
                  ...
                  onChange={(e: ChangeEvent<HTMLInputElement>) =>
                    handleEditingAreaChange('description', e.target.value)
                  }
                  placeholder="Description"
                  className="flex-1 bg-slate-800 border border-slate-600 rounded px-2 py-1 text-white text-sm"
                />
                <input
                  ...
                  onChange={(e: ChangeEvent<HTMLInputElement>) =>
                    handleEditingAreaChange('color', e.target.value)
                  }
