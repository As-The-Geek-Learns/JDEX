...
/**
 * Electron window with require
 */
interface ElectronWindow extends Window {
  require: (module: string) => { dialog?: ElectronDialog } | undefined;
}
...
export default function FileSelector({
  ...
  const setSafeError = useCallback((message: unknown): void => {
  ...
  const validateUserPath = useCallback(
    (inputPath: string | null): string | null => {
      ...
      try {
        ...
        setSafeError('Invalid path: ' + errorMsg);
        return null;
      }
    },
    [setSafeError]
  ...
  const loadFolder = useCallback(
    (path: string): void => {
      ...
      try {
        const files = readDirectoryFiles(path) as FileInfo[];
        ...
        const errorMsg = err instanceof Error ? err.message : 'Failed to read folder';
        setSafeError(errorMsg);
        setAvailableFiles([]);
      }
