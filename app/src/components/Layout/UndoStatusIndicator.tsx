...
import type { UndoAction } from '../../context/UndoContext.jsx';
 ...
 */
function getActionIcon(action: UndoAction | null): ReactElement | null {
...
/**
 * Action with display properties
 */
interface ActionWithDescription {
  description?: string;
  wasUndone?: boolean;
  wasRedone?: boolean;
}
 ...
 */
function formatDescription(action: ActionWithDescription | null): string {
  if (!action) return '';
 ...
 */
function UndoStatusIndicator(): JSX.Element | null {
        ...
        {lastAction && (
          <>
            ...
            <span className="text-slate-400 truncate max-w-48">
              {formatDescription(lastAction as ActionWithDescription)}
            </span>
          </>
        )}
      </div>
