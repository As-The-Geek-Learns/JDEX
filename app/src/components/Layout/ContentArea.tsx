 ...
 * =====================
 * Main content area displaying folders, items, and navigation.
 */
...
import type { FolderWithDisplay, ItemWithDisplay } from '../Cards/index.js';
import DropZone from '../DragDrop/DropZone.js';
import type { Area, Category, Folder } from '../../types/index.js';
import type { ViewType, BreadcrumbItem } from '../../hooks/useNavigation.js';
...
/**
 * Stats for overview - matches QuickStats interface
 */
interface Stats {
  totalFolders: number;
  totalItems: number;
...
export interface ContentAreaProps {
  ...
  stats: Stats;
  displayFolders: FolderWithDisplay[];
  displayItems: ItemWithDisplay[];
  onNavigate: (view: ViewType, data?: Area | Category | Folder | null) => void;
  onEditFolder: (folder: FolderWithDisplay) => void;
  onDeleteFolder: (folder: FolderWithDisplay) => void;
  onEditItem: (item: ItemWithDisplay) => void;
  onDeleteItem: (item: ItemWithDisplay) => void;
  onNewFolder: () => void;
  onNewItem: () => void;
  onRefresh: () => void;
}
...
function ContentArea({
  ...
  const handleOpenFolder = useCallback(
    (folder: FolderWithDisplay) => onNavigate('folder', folder),
    [onNavigate]
  );
        ...
        <div className="mb-8">
          ...
          <div className="space-y-3">
            {displayFolders.map((folder) => (
              <DropZone key={folder.id} folder={folder} onSuccess={onRefresh} onError={() => {}}>
                <FolderCard
                  folder={folder}
                  onEdit={onEditFolder}
