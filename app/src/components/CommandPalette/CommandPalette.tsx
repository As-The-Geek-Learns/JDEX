...
/**
 * Command handlers passed from parent.
 */
...
/**
 * Command with match score for filtering.
 */
 ...
 */
const createCommands = (handlers: CommandHandlers): Command[] => [
  // Create commands
  {
    id: 'newFolder',
    label: 'New Folder',
 ...
 */
function fuzzyMatch(query: string, target: string): FuzzyMatchResult {
 ...
 */
function filterCommands(commands: Command[], query: string): ScoredCommand[] {
  if (!query.trim()) {
    return commands.map((cmd) => ({ ...cmd, matches: true, score: 0 }));
  }
 ...
 */
function CommandPalette({ isOpen, onClose, handlers }: CommandPaletteProps): JSX.Element | null {
  ...
  const groupedCommands = filteredCommands.reduce<Record<string, ScoredCommand[]>>((acc, cmd) => {
    if (!acc[cmd.category]) {
      acc[cmd.category] = [];
    }
            ...
            Object.entries(groupedCommands).map(([category, cmds]) => (
              <div key={category}>
                ...
                {cmds.map((cmd) => {
                  ...
                  const Icon = cmd.icon;
                  // Only some commands have shortcuts - safely cast for lookup
                  const shortcut = getShortcutForAction(cmd.id as ShortcutAction);
