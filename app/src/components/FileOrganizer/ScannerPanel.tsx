...
/**
 * Props for QuickAccessFolders
 */
interface QuickAccessFoldersProps {
  onSelect: (path: string) => void;
}

/**
 * Props for FolderBrowser
 */
...
/**
 * Props for ScanProgress component
 */
...
/**
 * Props for ScanResults
 */
interface ScanResultsProps {
  sessionId: string;
  stats: ScanStats;
  onProceed: (sessionId: string, files: ScannedFile[]) => void;
}

/**
 * Props for ScannerPanel
 */
export interface ScannerPanelProps {
  onScanComplete?: (sessionId: string, files: ScannedFile[]) => void;
}
...
const FILE_TYPE_STYLES: Record<FileTypeId, FileTypeStyle> = {
 ...
 */
function QuickAccessFolders({ onSelect }: QuickAccessFoldersProps): JSX.Element | null {
  const [drives, setDrives] = useState<CloudDrive[]>([]);

  useEffect(() => {
    const cloudDrives = getCloudDrives() as CloudDrive[];
 ...
 */
function FolderBrowser({ currentPath, onNavigate }: FolderBrowserProps): JSX.Element | null {
  const [subdirs, setSubdirs] = useState<SubdirectoryInfo[]>([]);
  const [expanded, setExpanded] = useState(true);

  useEffect(() => {
    if (currentPath) {
      const dirs = listSubdirectories(currentPath) as SubdirectoryInfo[];
      setSubdirs(dirs);
    }
  }, [currentPath]);
 ...
 */
function ScanProgressComponent({ progress, onCancel }: ScanProgressProps): JSX.Element {
 ...
 */
function ScanResults({ sessionId, stats, onProceed }: ScanResultsProps): JSX.Element {
  ...
  useEffect(() => {
    if (sessionId) {
      try {
        ...
        // Group by file type
        const grouped = scannedFiles.reduce<Record<string, ScannedFile[]>>((acc, file) => {
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        {sortedTypes.map((type) => {
          const style = FILE_TYPE_STYLES[type as FileTypeId] || FILE_TYPE_STYLES.other;
...
export default function ScannerPanel({ onScanComplete }: ScannerPanelProps): JSX.Element {
  // State
  const [scanPath, setScanPath] = useState('');
  const [isScanning, setIsScanning] = useState(false);
  const [progress, setProgress] = useState<ServiceScanProgress | null>(null);
  ...
  const handleStartScan = useCallback(async (): Promise<void> => {
    ...
    const result = await scanner.scan(scanPath, {
      onProgress: (prog: ServiceScanProgress) => {
        setProgress(prog);
      },
      onFile: () => {
    ...
    if (result.success) {
      ...
      const errorMsg = result.error instanceof Error ? result.error : String(result.error);
  ...
  const handleBrowse = useCallback(async (): Promise<void> => {
    if (typeof window !== 'undefined' && (window as Window & { require?: unknown }).require) {
      try {
  ...
  const handleProceed = useCallback(
    (sessionId: string, files: ScannedFile[]): void => {
      if (onScanComplete) {
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                type="text"
                value={scanPath}
                onChange={(e: ChangeEvent<HTMLInputElement>) => setScanPath(e.target.value)}
