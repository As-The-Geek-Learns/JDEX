 ...
 * ====================
 * Displays a folder card with expandable details.
 *
 * WHAT: Folder display card with edit/delete actions.
 * WHY: Provides a consistent, visually appealing card UI for folders.
 */
...
/**
 * Extended folder type with display properties from joined tables.
 */
export interface FolderWithDisplay extends Folder {
  area_color?: string;
  area_name?: string;
...
export interface FolderCardProps {
  folder: FolderWithDisplay;
  onEdit: (folder: FolderWithDisplay) => void;
  onDelete: (folder: FolderWithDisplay) => void;
  onOpen: (folder: FolderWithDisplay) => void;
}
...
/**
 * FolderCard component wrapped with React.memo for performance optimization.
 * Only re-renders when folder data changes (callbacks are excluded from comparison
 * since they typically change reference but not behavior).
 */
const FolderCard = memo(
  function FolderCard({ folder, onEdit, onDelete, onOpen }: FolderCardProps): JSX.Element {
        ...
        <div className="relative flex items-start justify-between">
          <div className="flex-1 min-w-0" onClick={() => onOpen(folder)}>
            <div className="flex items-center gap-3 mb-2">
              ...
              <SensitivityBadge sensitivity={folder.sensitivity as Sensitivity | undefined} />
            </div>
  ...
  (prevProps, nextProps) => {
    // Custom comparison: only re-render if folder data changed
    // Callbacks are intentionally excluded since they often change reference
    // but not behavior, and the parent should use useCallback if needed
    return (
      prevProps.folder.id === nextProps.folder.id &&
      prevProps.folder.folder_number === nextProps.folder.folder_number &&
      prevProps.folder.name === nextProps.folder.name &&
      prevProps.folder.description === nextProps.folder.description &&
      prevProps.folder.sensitivity === nextProps.folder.sensitivity &&
      prevProps.folder.location === nextProps.folder.location &&
      prevProps.folder.storage_path === nextProps.folder.storage_path &&
      prevProps.folder.keywords === nextProps.folder.keywords &&
      prevProps.folder.notes === nextProps.folder.notes &&
      prevProps.folder.area_color === nextProps.folder.area_color &&
      prevProps.folder.area_name === nextProps.folder.area_name &&
      prevProps.folder.category_name === nextProps.folder.category_name
    );
  }
);
